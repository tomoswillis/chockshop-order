    ...
    <div class="w-full checkoutImg relative">
      ...
      >
        ...
        <div class="lg:w-2/3" v-if="cart">
          ...
          <div class="mt-12">
            <inertia-link
              :href="route('clear.cart')"
              method="post"
              class="bg-chock align-center w-full rounded-xl font-bold py-2 px-5 text-chock-text text-sm cursor-pointer border border-transpartent hover:border-chock hover:bg-chock-dark hover:text-chock"
              >Clear Basket</inertia-link
            >
          </div>
          <div
          ...
          >
            ...
            <inertia-link
              v-if="item.item.id"
              method="Post"
              :href="route('product.removeFromCart', item.item.id)"
              as="button"
            >
              <div
                class="hover:bg-red-300 p-3 rounded hover:text-red-900 text-white"
              >
                <svg
        ...
        >
          <div class="flex flex-wrap -mx-2 mt-8">
            <div class="p-2 w-1/2">
              <div class="relative">
                <label
                  for="first_name"
                  class="leading-7 text-sm text-chock-text"
                  >First Name</label
                >
                <input
                  type="text"
            ...
            <div class="p-2 w-1/2">
              <div class="relative">
                <label for="last_name" class="leading-7 text-sm text-chock-text"
                  >Last Name</label
                >
                <input
                  type="text"
          ...
          <div class=" ">
            <div class="relative">
              <label for="email" class="leading-7 text-sm text-chock-text"
                >Email Address</label
              >
              <input
                type="email"
...
export default {
  ...
  async mounted() {
    this.stripe = await loadStripe(
      "pk_test_51HxJevHJuC8e8ZRStp3O54491bNJjRoqQs0CH1djjVO2RRmbRXXIOTIdxl7VUrVHUOV22echJoCRUEK3rbezU4IN00RU1zjr3Z"
    );
